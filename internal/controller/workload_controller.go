...
func (r *WorkloadReconciler) reconcileCompute(ctx context.Context, workload *calypsov1alpha1.Workload, ns string) error {
	name := fmt.Sprintf("%s-compute", workload.Name)
	var compute calypsov1alpha1.Compute

	err := r.Get(ctx, client.ObjectKey{Namespace: ns, Name: name}, &compute)
	if err != nil {
		if apierrors.IsNotFound(err) {
			if workload.Spec.Compute == nil {
				return nil
			}
			// Create it
			...
			if err := ctrl.SetControllerReference(workload, newCompute, r.Scheme); err != nil {
				return fmt.Errorf("failed to set controller reference on compute: %w", err)
			}
			return r.Create(ctx, newCompute)
		}
		return err
	}

	if workload.Spec.Compute == nil {
		return r.Delete(ctx, &compute)
	}

	compute.Spec = *workload.Spec.Compute
	compute.Spec.TargetRef = &workload.Spec.TargetRef
	return r.Update(ctx, &compute)
}

func (r *WorkloadReconciler) reconcileStorage(ctx context.Context, workload *calypsov1alpha1.Workload, ns string) error {
	name := fmt.Sprintf("%s-storage", workload.Name)
	var storage calypsov1alpha1.Storage

	err := r.Get(ctx, client.ObjectKey{Namespace: ns, Name: name}, &storage)
	if err != nil {
		if apierrors.IsNotFound(err) {
			if workload.Spec.Storage == nil {
				return nil
			}
			...
			if err := ctrl.SetControllerReference(workload, newStorage, r.Scheme); err != nil {
				return fmt.Errorf("failed to set controller reference on storage: %w", err)
			}
			return r.Create(ctx, newStorage)
		}
		return err
	}

	if workload.Spec.Storage == nil {
		return r.Delete(ctx, &storage)
	}
...
func (r *WorkloadReconciler) reconcileNetworking(ctx context.Context, workload *calypsov1alpha1.Workload, ns string) error {
	name := fmt.Sprintf("%s-networking", workload.Name)
	var networking calypsov1alpha1.Networking

	err := r.Get(ctx, client.ObjectKey{Namespace: ns, Name: name}, &networking)
	if err != nil {
		if apierrors.IsNotFound(err) {
			if workload.Spec.Networking == nil {
				return nil
			}
			...
			if err := ctrl.SetControllerReference(workload, newNetworking, r.Scheme); err != nil {
				return fmt.Errorf("failed to set controller reference on networking: %w", err)
			}
			return r.Create(ctx, newNetworking)
		}
		return err
	}

	if workload.Spec.Networking == nil {
		return r.Delete(ctx, &networking)
	}

	networking.Spec = *workload.Spec.Networking
	networking.Spec.TargetRef = &workload.Spec.TargetRef
	return r.Update(ctx, &networking)
}

func (r *WorkloadReconciler) reconcileObservability(ctx context.Context, workload *calypsov1alpha1.Workload, ns string) error {
	name := fmt.Sprintf("%s-observability", workload.Name)
	var observability calypsov1alpha1.Observability

	err := r.Get(ctx, client.ObjectKey{Namespace: ns, Name: name}, &observability)
	if err != nil {
		if apierrors.IsNotFound(err) {
			if workload.Spec.Observability == nil {
				return nil
			}
			...
			if err := ctrl.SetControllerReference(workload, newObservability, r.Scheme); err != nil {
				return fmt.Errorf("failed to set controller reference on observability: %w", err)
			}
			return r.Create(ctx, newObservability)
		}
		return err
	}

	if workload.Spec.Observability == nil {
		return r.Delete(ctx, &observability)
	}

	observability.Spec = *workload.Spec.Observability
	observability.Spec.TargetRef = &workload.Spec.TargetRef
	return r.Update(ctx, &observability)
}

func (r *WorkloadReconciler) reconcileSecurity(ctx context.Context, workload *calypsov1alpha1.Workload, ns string) error {
	name := fmt.Sprintf("%s-security", workload.Name)
	var security calypsov1alpha1.Security

	err := r.Get(ctx, client.ObjectKey{Namespace: ns, Name: name}, &security)
	if err != nil {
		if apierrors.IsNotFound(err) {
			if workload.Spec.Security == nil {
				return nil
			}
			...
			if err := ctrl.SetControllerReference(workload, newSecurity, r.Scheme); err != nil {
				return fmt.Errorf("failed to set controller reference on security: %w", err)
			}
			return r.Create(ctx, newSecurity)
		}
		return err
	}

	if workload.Spec.Security == nil {
		return r.Delete(ctx, &security)
	}

	security.Spec = *workload.Spec.Security
	security.Spec.TargetRef = &workload.Spec.TargetRef
	return r.Update(ctx, &security)
}
